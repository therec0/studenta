...
(function () {
    ...
    function setupAnalyzer(container) {
        ...
        // --- Reuse parsing, table update, highlighting, popup logic ---
        ...
        function parseTable() {
            const tb = container.querySelector('table tbody');
            if (!tb) return { lessons: {}, rows: [] };
        ...
        function updateForSelectedSubject(){
            ...
            tryShowAllAndWait(()=>{
                ...
                statsDiv.innerHTML = `<b>${chosen}</b><br>
                  Läsnä: ${lasnaH} h<br>
                  Poissa: ${poissaH} h<br>
                ...
                if(data.poissaDates.length){
                    const lines = data.poissaDates.map(d=>`<div>${d.date} = ${(d.mins/60).toFixed(1)} h</div>`);
